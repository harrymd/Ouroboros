...
function prepare_model_dictionary_toroidal(mu, anelastic_params, dir_numpy, i)

    model_type = anelastic_params["layer_model"][i + 1]

    if model_type == "SLS_uniform"
    ...
    elseif model_type == "extended_burgers_uniform"
...
function toroidal_rep(args)
    ...
    dir_numpy, dir_julia, l, Ki, dimension, A, mu, B, xx = 
        read_numpy_files_toroidal(dir_output, i)
    ...
    model = prepare_model_dictionary_toroidal(mu, anelastic_params, dir_numpy, i)
    ...
    if anelastic_params["layer_model"][i + 1] in [0, -2, "extended_burgers_uniform"]

        # This is a zero matrix.
        A1 = temp_A0

    # All other cases have REP terms.
    elseif anelastic_params["layer_model"][i + 1] in ["maxwell_uniform", "SLS_uniform",
                                              "burgers_uniform", "SLS"]
        ...
        error_str = @sprintf("Model type %s not recognised.",
                             anelastic_params["layer_model"][i + 1])
        error(error_str)
    ...
    for k = 1 : Ki
        ...
        roots, poles = update_root_pole_list(roots, poles,
                            anelastic_params["layer_model"][i + 1], root_pole_k)
    ...
    solve_NEP_wrapper(nep, anelastic_params, poles, roots, l, i,
